import { sendVerificationEmail, sendWelcomeEmail, sendResetPasswordEmail } from "../middlewares/Email.js";
...
const Login = async (req, res) => {
  try {
    ...
    // 5. Generate token
    const token = generateTokenAndSetCookies(res, user)

    return res.status(200).json({
      success: true,
      message: "Login successful",
      token: token, // ✅ TOKEN भेजें frontend के लिए
      user: {
        id: user._id,
        name: user.name,
        email: user.email,
        role: user.role || 'user' // ✅ role जरूर भेजें
      }
    })
...
const Logout = (req, res) => {
  try {
    // agar cookie based auth hai
    res.clearCookie("token"); // cookie ka naam jo tum use kar rahi ho
...
export { checkLogin, Register, VerifyEmail, Login, ForgotPassword, ResetPassword, VerifyResetOTP, Logout }